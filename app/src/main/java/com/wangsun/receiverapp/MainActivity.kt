...
class MainActivity : AppCompatActivity() {
    ...
    private fun initButton() {
        id_send_broadcast_test.setOnClickListener {
            /*test broadcast*/
            val intent = Intent(Constants.Actions.BROADCAST_ACTION_NAME2)
        ...
        id_send_broadcast_uid.setOnClickListener {
            /*uid broadcast*/
            val intent = Intent(Constants.Actions.BROADCAST_ACTION_USER_ID2)
            intent.addCategory(Intent.CATEGORY_DEFAULT)
            intent.putExtra(Constants.IntentKey.UID_KEY,"hakhun")
            sendBroadcast(intent)
        }
    }
}
